  ...
  return (
    <div className="rounded-2xl border border-white/15 bg-white/5 overflow-hidden">
      <button
        ...
        className="w-full flex items-center justify-between px-5 py-5 hover:bg-white/5 transition-colors"
      >
...
const Field: React.FC<FieldProps> = ({ id, label, type = 'text', value, onChange, maxLength, pattern }) => (
  <div className="flex flex-col gap-2">
    <label htmlFor={id} className="text-sm text-gray-400 uppercase tracking-wider font-bold">{label}</label>
    <input
      id={id}
      type={type}
      value={value}
      ...
      maxLength={maxLength}
      pattern={pattern}
      className="bg-white/5 border border-white/15 rounded-xl px-4 py-4 text-base text-foreground placeholder-gray-500 focus:outline-none focus:border-red-500/50 transition-colors"
    />
  </div>
);
  ...
  return (
    <div className="flex flex-col gap-5 py-3">
      ...
      >
        <div className="flex items-center gap-3">
          {aDesContacts
            ? <Phone size={22} className="text-green-400 shrink-0" />
            : <AlertTriangle size={22} className="text-red-400 shrink-0" />}
          <div className="flex flex-col items-start gap-1">
            <span className={`text-lg font-bold ${aDesContacts ? 'text-white' : 'text-red-300'}`}>
      ...
      <Accordion icon={<User size={16} />} title={t('settings.profile')} sectionKey="profile" openKey={openSection} onToggle={toggleSection}>
        ...
        />
        <div className="flex flex-col gap-2">
          <label htmlFor="settings-email" className="text-sm text-gray-400 uppercase tracking-wider font-bold">
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 rounded-xl bg-black/30 border border-white/15 text-white text-base px-4 py-4 placeholder:text-gray-500 focus:outline-none focus:border-red-500/50"
            />
            <button
              onClick={() => {
      ...
      <Accordion icon={<Globe size={16} />} title={t('settings.language')} sectionKey="language" openKey={openSection} onToggle={toggleSection}>
        <p className="text-sm text-gray-400 leading-relaxed">{t('settings.languageDesc')}</p>
        <button
          onClick={() => setLangOpen(!langOpen)}
          className="flex items-center justify-between w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3 hover:bg-white/8 transition-all"
        >
          <div className="flex items-center gap-2.5">
            <span className="text-xl">{currentLang?.flag ?? '🌐'}</span>
            <span className="text-sm text-white font-medium">{currentLang?.nativeName ?? i18n.language}</span>
          </div>
          <ChevronRight size={14} className={`text-gray-600 transition-transform ${langOpen ? 'rotate-90' : ''}`} />
        </button>
        {langOpen && (
          <div className="flex flex-col gap-1 max-h-60 overflow-y-auto rounded-xl border border-white/10 bg-black/30 p-1">
            {LANGUAGES.map((lang) => (
              <button
                key={lang.code}
              ...
              >
                ...
                {(i18n.language?.split('-')[0] || 'fr') === lang.code && (
                  <CheckCircle size={14} className="text-red-400 ml-auto" />
                )}
              </button>
            ))}
          </div>
      ...
      <Accordion icon={<Lock size={16} />} title={t('settings.changePassword')} sectionKey="password" openKey={openSection} onToggle={toggleSection}>
        ...
        />
        {pwdMsg && (
          <p className={`text-xs font-medium ${pwdMsg.type === 'ok' ? 'text-green-400' : 'text-red-400'}`}>
            {pwdMsg.text}
          </p>
        )}
        <motion.button
      ...
      <Accordion icon={<Shield size={16} />} title={t('settings.security')} sectionKey="codes" openKey={openSection} onToggle={toggleSection}>
        ...
        />
        <div className="rounded-xl bg-red-950/40 border border-red-900/30 p-3">
      ...
      <div className="rounded-2xl border border-white/15 bg-white/5 px-5 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          {notifPermission === 'granted'
            ? <Bell size={20} className="text-green-400 shrink-0" />
            : <BellOff size={20} className="text-gray-500 shrink-0" />}
          <span className="text-base font-bold text-foreground">{t('settings.notifications')}</span>
        </div>
        {notifPermission === 'granted' ? (
          <span className="text-sm font-bold text-green-400 uppercase">{t('common.enabled')}</span>
        ) : notifPermission === 'denied' ? (
          <span className="text-sm font-bold text-red-400 uppercase">{t('common.refused')}</span>
        ) : (
          <button
        ...
        <div className="rounded-2xl p-5 flex flex-col gap-4"
            ...
            <button onClick={() => setView('sponsor')}
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div className="flex flex-col gap-0.5 items-start">
                  <span className="text-sm font-bold text-white">{t('settings.platinum.sponsorship')}</span>
          ...
          {isGuest && user.sponsorLink && (
            <div className="flex items-center gap-3 rounded-xl p-3.5"
              ...
              <div className="flex flex-col gap-0.5">
                <span className="text-sm font-bold text-white">{t('settings.platinum.guestPlatinum')}</span>
                <span className="text-sm" style={{ color: 'rgba(255,215,90,0.55)' }}>
      ...
      <div className="flex flex-col gap-2">
        {[
          { key: 'emergency-numbers',  icon: <Phone size={22} className="text-green-400" />,   label: t('emergencyNumbers.title'), pro: false },
          { key: 'medical',            icon: <Heart size={22} className="text-red-400" />,    label: t('medical.title'),        pro: true },
          { key: 'qrcode',             icon: <QrCode size={22} className="text-red-400" />,   label: t('qrcode.title'),         pro: true },
          { key: 'journal',            icon: <BookOpen size={22} className="text-red-400" />,  label: t('journal.title'),        pro: true },
          { key: 'meeting',            icon: <Users size={22} className="text-red-400" />,     label: t('meeting.title'),        pro: true },
          { key: 'travel',             icon: <Plane size={22} className="text-red-400" />,     label: t('travel.title'),         pro: true },
        ].map((item) => (
          <button
            key={item.key}
            onClick={() => setView(item.key as any)}
            className="flex items-center justify-between w-full rounded-2xl border border-white/15 bg-white/5 px-5 py-4 hover:bg-white/8 transition-all"
          >
            <div className="flex items-center gap-3">
              {item.icon}
            ...
            <div className="flex items-center gap-2">
              {item.pro && !(isPro || isPlatinum) && <Lock size={16} className="text-gray-600" />}
              <ChevronRight size={20} className="text-gray-500" />
            </div>
          </button>
        ))}
      ...
      {(isPro || isPlatinum) && (
        <Accordion icon={<PhoneCall size={16} className="text-green-400" />} title={t('home.planTable.fakeCallTitle')} sectionKey="fakecall" openKey={openSection} onToggle={toggleSection}>
          <p className="text-sm text-gray-500 leading-relaxed">
            {t('home.planTable.fakeCallDesc')}
          </p>

          <div className="flex flex-col gap-1.5">
            <label className="text-xs text-gray-500 uppercase tracking-wider">{t('home.planTable.fakeCallCallerName')}</label>
            <input
              type="text"
              ...
              className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-foreground placeholder-gray-600 focus:outline-none focus:border-green-500/50 transition-colors"
            />
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            />
            <span className="text-sm text-gray-600">{t('home.planTable.fakeCallPhoneHint')}</span>
          </div>

          <div className="flex flex-col gap-1.5">
            <label className="text-xs text-gray-500 uppercase tracking-wider">{t('home.planTable.fakeCallDelay')}</label>
            <input
              type="number"
              min={5}
              max={300}
              ...
              className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-foreground placeholder-gray-600 focus:outline-none focus:border-green-500/50 transition-colors"
            />
            <span className="text-sm text-gray-600">{t('home.planTable.fakeCallDelayHint')}</span>
          </div>

          <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-green-950/20 border border-green-500/15">
            <PhoneCall size={11} className="text-green-400 shrink-0" />
            <span className="text-sm text-green-300 leading-relaxed">
      ...
      {(isPro || isPlatinum) && (
        <Accordion icon={<Brain size={16} className="text-blue-400" />} title="Mode Alzheimer" sectionKey="alzheimer" openKey={openSection} onToggle={toggleSection}>
          <p className="text-sm text-gray-500 leading-relaxed">
          ...
          <div className="flex items-center justify-between rounded-xl p-3.5 bg-white/[0.03] border border-white/8">
            <div className="flex items-center gap-2.5">
              ...
              <div className="flex flex-col gap-0.5">
                ...
                <span className="text-sm text-gray-500">Page d'identification simplifiée</span>
              </div>
            </div>
          ...
          {alzheimerMode.enabled && (
            <div className="flex flex-col gap-3">
              <div className="flex flex-col gap-1.5">
                <label className="text-xs text-gray-500 uppercase tracking-wider">Nom affiché (sur la page publique)</label>
                <input
                  type="text"
                  ...
                  className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-foreground placeholder-gray-600 focus:outline-none focus:border-blue-500/50 transition-colors"
                />
              </div>

              <div className="flex flex-col gap-1.5">
                <label className="text-xs text-gray-500 uppercase tracking-wider">Nom du tuteur / aidant</label>
                <input
                  type="text"
              ...
              <div className="flex items-center justify-between rounded-xl p-3.5 bg-white/[0.03] border border-white/8">
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="flex flex-col gap-0.5">
                    ...
                    <span className="text-sm text-gray-500">Cache l'adresse exacte aux inconnus</span>
                  </div>
                </div>
          ...
          {isPlatinum ? (
            <button onClick={() => setView('alzheimer-sponsor')}
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div className="flex flex-col gap-0.5 items-start">
                  <span className="text-sm font-bold text-white">{t('alzheimerSponsor.title')}</span>
            ...
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-purple-950/15 border border-purple-500/15">
      ...
      <button onClick={() => setView('legal')}
        className="flex items-center justify-between w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3.5 hover:bg-white/8 transition-all">
        <div className="flex items-center gap-2">
          <FileText size={15} className="text-gray-500" />
          <span className="text-sm text-gray-400">{t('legal.terms')}</span>
        </div>
      ...
      >
        <div className="flex items-center gap-3">
          <HelpCircle size={22} className={isPlatinum ? 'text-yellow-400' : 'text-red-400'} />
          <span className="text-base font-bold text-gray-200">{t('settings.helpTitle')}</span>
        </div>
        <ChevronRight size={20} className="text-gray-500" />
      </button>
      ...
      <motion.button onClick={handleLogout}
        className="btn-3d btn-3d-slate flex items-center justify-center gap-3 py-5 rounded-2xl font-bold text-lg text-gray-200 tracking-wide transition-all"
        whileTap={{ scale: 0.97 }}>
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 flex flex-col gap-3">
      <div className="flex items-center gap-2">
        ...
        <span className="text-base font-bold text-gray-200">{t('settings.subscription.title')}</span>
      </div>
      <div className="flex items-center gap-2">
        <Crown size={14} className={isPlatinum ? 'text-yellow-400' : 'text-green-400'} />
        <span className={`text-sm font-bold ${isPlatinum ? 'text-yellow-400' : 'text-green-400'}`}>
      ...
      <AnimatePresence>
        {done && (
          <motion.div className="flex items-center gap-2 px-3 py-2 rounded-xl border border-green-500/30 bg-green-950/20"
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <CheckCircle size={14} className="text-green-400" />
            <span className="text-xs text-green-300 font-bold">{done}</span>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        <div className="flex flex-col gap-2">
          {isPlatinum && (
            <motion.button whileTap={{ scale: 0.97 }}
              ...
              className="w-full flex items-center justify-center gap-2 py-3.5 rounded-xl border border-yellow-500/25 bg-yellow-950/15 text-yellow-400 font-bold text-sm">
              <ArrowDownCircle size={16} /> {t('upgrade.downgradeToProTitle')}
            </motion.button>
          )}
          <motion.button whileTap={{ scale: 0.97 }}
            onClick={() => setShowConfirm('cancel')}
            className="w-full flex items-center justify-center gap-2 py-3.5 rounded-xl border border-red-500/25 bg-red-950/15 text-red-400 font-bold text-sm">
      ...
      <AnimatePresence>
        {showConfirm === 'cancel' && (
          <motion.div className="flex flex-col gap-3 p-3 rounded-xl border border-red-500/30 bg-red-950/20"
            initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}>
            <div className="flex items-center gap-2">
              <AlertTriangle size={14} className="text-red-400" />
              <span className="text-sm font-bold text-red-300">{t('upgrade.cancelSubscription')}</span>
            </div>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => setShowConfirm(null)}
                className="flex-1 py-2.5 rounded-xl bg-white/10 text-gray-400 text-sm font-bold">
                {t('common.cancel')}
              </button>
            </div>
          </motion.div>
        ...
        {showConfirm === 'downgrade' && (
          <motion.div className="flex flex-col gap-3 p-3 rounded-xl border border-yellow-500/30 bg-yellow-950/20"
            initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}>
            <div className="flex items-center gap-2">
              <AlertTriangle size={14} className="text-yellow-400" />
              <span className="text-sm font-bold text-yellow-300">{t('upgrade.downgradeToProTitle')}</span>
            </div>
