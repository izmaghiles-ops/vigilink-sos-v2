/**
 * OnboardingView — Configuration initiale Vigilink-SOS
 *
 * Étapes : welcome → contact → setup → legal
 *
 * L'étape "profile" a été supprimée car le nom et téléphone
 * sont déjà collectés lors de l'inscription (LoginView).
 *
 * Admin : Giles
 */
...
function StatusIcon({ status }: { status: TestStatus }) {
  if (status === 'testing') return <Loader size={16} className="text-yellow-400 animate-spin" />;
  ...
  if (status === 'unavailable') return <XCircle size={16} className="text-gray-500" />;
  return <div className="w-4 h-4 rounded-full border border-white/20 bg-white/5" />;
}
...
export const OnboardingView: React.FC<OnboardingViewProps> = ({ onComplete }) => {
  ...
  const BackButton = () => (
    <motion.button
      onClick={goBack}
      className="flex items-center gap-2 text-gray-400 hover:text-white transition-colors py-2 -ml-1"
      whileTap={{ scale: 0.95 }}
    >
      <ChevronLeft size={20} />
      ...
      <div className="w-full max-w-sm flex flex-col gap-6">
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 'welcome' && (
            ...
            >
              <motion.div
                className="w-24 h-24 rounded-3xl bg-red-900/30 border border-red-500/30 flex items-center justify-center"
                animate={{ scale: [1, 1.04, 1] }}
                transition={{ repeat: Infinity, duration: 2.5 }}
              >
                <Shield size={42} className="text-red-500" />
              </motion.div>

              <div className="flex flex-col items-center gap-2 text-center">
                <h1 className="text-3xl font-black text-white tracking-wider uppercase">{t('app.name')}</h1>
              ...
              <motion.button
                ...
                className="w-full flex items-center justify-center gap-2 py-4 rounded-2xl bg-red-600 hover:bg-red-500 text-white font-black text-base transition-all shadow-lg shadow-red-900/40"
                whileTap={{ scale: 0.98 }}
              >
            ...
            >
              ...
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-gray-500 uppercase tracking-widest">{t('onboarding.contact.title')} (1/3)</span>
                </div>
                <h2 className="text-xl font-black text-white">{t('onboarding.contact.title')}</h2>
                <p className="text-xs text-gray-600">{t('onboarding.contact.subtitle')}</p>
              </div>

              <div className="flex flex-col gap-3">
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs text-gray-500 uppercase tracking-wider">{t('onboarding.contact.nameLabel')} *</label>
                  <input
                    type="text"
                    placeholder={t('onboarding.contact.namePlaceholder')}
                    value={contact.name}
                    onChange={(e) => setContact((c) => ({ ...c, name: e.target.value }))}
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3.5 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-red-500/50 transition-colors"
                    autoFocus
                  />
                </div>
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs text-gray-500 uppercase tracking-wider">{t('onboarding.contact.relationship')}</label>
                  <input
                    type="text"
              ...
              <motion.button
                ...
                className="flex items-center justify-center gap-2 py-4 rounded-2xl bg-red-600 hover:bg-red-500 disabled:opacity-40 text-white font-black transition-all"
                whileTap={{ scale: 0.98 }}
              >
            ...
            >
              ...
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-gray-500 uppercase tracking-widest">{t('onboarding.permissions.title')} (2/3)</span>
                </div>
                <h2 className="text-xl font-black text-white">{t('onboarding.permissions.title')}</h2>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex flex-col gap-0.5 flex-1 min-w-0">
                        <span className="text-sm font-black text-white">{t(test.labelKey)}</span>
                        <span className="text-sm text-gray-500 leading-relaxed"
                              dangerouslySetInnerHTML={{ __html: t(test.hintKey) }} />
                        <span className={[
                          'text-sm font-bold mt-0.5',
                          test.status === 'ok'          ? 'text-green-400'
                          : test.status === 'denied'    ? 'text-red-400'
                          : test.status === 'testing'   ? 'text-yellow-400'
                          : test.status === 'unavailable' ? 'text-gray-500'
                          : 'text-gray-600',
                        ].join(' ')}>
                      ...
                      <div className="shrink-0">
                        {isDone ? (
                          ...
                          <motion.button
                            ...
                            disabled={isTesting}
                            className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-red-700 hover:bg-red-600 disabled:opacity-50 text-white text-sm font-black uppercase tracking-wider transition-all"
                            whileTap={{ scale: 0.96 }}
                          >
              ...
              {tests.some((t) => t.status === 'denied') && (
                <motion.div
                  className="rounded-xl border border-yellow-500/20 bg-yellow-950/10 px-4 py-3"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                >
              ...
              <motion.button
                ...
                className="flex items-center justify-center gap-2 py-4 rounded-2xl bg-red-600 hover:bg-red-500 disabled:opacity-40 disabled:cursor-not-allowed text-white font-black transition-all"
                whileTap={{ scale: 0.98 }}
              >
                {t('common.next')} <ChevronRight size={18} />
              </motion.button>
          ...
          {step === 'legal' && (
            ...
            >
              ...
              <div className="flex items-center gap-2 mb-4">
                <Shield size={16} className="text-red-400" />
                <span className="text-xs text-gray-500 uppercase tracking-widest">{t('onboarding.legal.title')} (3/3)</span>
              </div>
